...
function myClick() {
    ...
    let nextGame = document.getElementById('next_game');
    nextGame.style.display = 'block'; //активируем кнопку Начать игру сначала
    ...
    playfield.addEventListener ('click', event => {
        if (event.target.className == 'play_square') 
        {
            ...
            let draw = resultСheck(arr, i, j, arr[i][j]); //проверка количество ходов игроков и выявление Победителя после каждого шага
